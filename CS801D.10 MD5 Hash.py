...
#5. Break P into chunks of 512 bits; lets call the chunks P[0], P[1]... P[N-1]. Total length is say L = 512*N.
#6. Define 4 functions, each taking 3 arguments: F(X,Y,Z), G(X,Y,Z), H(X,Y,Z), I(X,Y,Z) such that:
...
'''
# Process each 16-word block (1 word = 16 bits, message has 512*N bits so 31*N words. 16 word = 256 bit block).
# Let L be length = 512*N and LL be the number of 32 bit words in the padded message.
T[] = set of 64 unique constants.
For i = 0 to (LL/16)-1 do:

	# Copy block i into X => For round 1, X[0-15] = P[0-15], for round 2, X[0-15] = P[16-31] and so on.
	For j = 0 to 15 do
		X[j] := P[i*16+j].
	End of j loop.
...
End of i loop.
'''
#8. Append ABCD to form the output.
